 ...
 */
static int8_t
prvGetNumberOfParameters(const char *pcCommandString);
...
static const CLI_Command_Definition_t xHelpCommand = {
		...
		"\r\n"
		"help: \r\n"
		"  Short command line interface documentation.\r\n",
		prvHelpCommand,
		0
};
...
static CLI_Definition_List_Item_t xRegisteredCommands = {
		/* The first command in the list is always the help command, defined in
		 * this file. */
		&xHelpCommand,
		/* The next pointer is initialised to NULL, as there are no other
 ...
 * #define cmdQUEUE_LENGTH access to the command interpreter by UART and by
 * Ethernet.  Sharing a buffer is done purely to save RAM.  Note, however, that
 * the command console itself is not re-entrant, so only one command interpreter
 * interface can be used at any one time.  For that reason, no attempt at
 * providing mutual exclusion to the cOutputBuffer array is attempted.
 *
...
FreeRTOS_CLIRegisterCommand(
	...
	if (pxNewListItem != NULL) {
		taskENTER_CRITICAL();
		{
			...
			/* Add the newly created list item to the end of the already
			 * existing list. */
...
FreeRTOS_CLIProcessCommand(
	...
	if ((pxCommand != NULL) && (xReturn == pdFALSE)) {
		...
		strncpy(pcWriteBuffer,
				...
				"Enter \"man <command>\" to view a list of parameters.\r\n",
				xWriteBufferLen
		);
		pxCommand = NULL;
		...
		BaseType_t *pxParameterStringLength)
{
	...
	*pxParameterStringLength = 0;
	...
	while (uxParametersFound < uxWantedParameter) {
		/* Index the character pointer past the current word.  If this is the
		 * start of the command string then the first word is the command
		 * itself. */
...
prvHelpCommand(
	...
	static const char * const pcTitle =
	...
	sprintf(pcWriteBuffer, pcTitle);
	pcWriteBuffer += (unsigned short) strlen(pcTitle);
	...
	for (pxCommand = &xRegisteredCommands;
		...
		pcWriteBuffer += (uint16_t) strlen(pcRegisteredCommandString) + 4;
	}
