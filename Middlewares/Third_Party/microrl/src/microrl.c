...
{
	static int esc_body = 0;

	switch (ch) {
		case _VT100_BRACKET_CODE:
			return 0;
		case _VT100_CURSOR_UP_CODE:
#ifdef _USE_HISTORY
			hist_search(pThis, _HIST_UP);
#endif
			return 1;
		case _VT100_CURSOR_DOWN_CODE:
#ifdef _USE_HISTORY
			hist_search(pThis, _HIST_DOWN);
#endif
			return 1;
		case _VT100_CURSOR_RIGHT_CODE:
			if (pThis->cursor < pThis->cmdlen) {
				terminal_move_cursor(pThis, 1);
				pThis->cursor++;
			}
			return 1;
		case _VT100_CURSOR_LEFT_CODE:
			if (pThis->cursor > 0) {
				terminal_move_cursor(pThis, -1);
				pThis->cursor--;
			}
			return 1;
		case _VT100_TILDA_CODE:
			if (esc_body == 1) {
				switch (pThis->escape_seq) {
					case _VT100_HOME_CODE:
						terminal_reset_cursor(pThis);
						pThis->cursor = 0;
						break;
					case _VT100_END_CODE:
